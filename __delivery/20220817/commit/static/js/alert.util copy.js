...
(function($) {
	...
	function focusNavigateTip(id,msg){
		...
		if(target$.parents('.beo-search-condition-parent').length > 0) { 
			target$.parents('.beo-search-condition').css("display","block");
		} 
	...
	function msgInit() {
		if( $('.beo-alert-wrap').length > 0 ) $('.beo-alert-wrap').remove();

		$('body').append(
		'	<div class="beo-alert-wrap">															'+
		...
		$('.beo-alert-wrap button').off('click');
	}
	...
	function alertAutoClose(message){
		...
		$('.beo-alert-wrap .auto-closeW-wrap').show();	
		var newMessage = message.replace('\n','<br>');
		$('.beo-alert-wrap .Massage-wrap').html(newMessage);

		$('.beo-alert-wrap #autoCloseBt').click(function(){
			$('.beo-alert-wrap .auto-closeW-wrap').hide();
		});
	...
	function alertDialog(message, callback){
		msgInit();
		$('.beo-alert-wrap .alertWindow-wrap').show();
		
		var newMessage = message.replace('\n','<br>');
		$('.beo-alert-wrap .Massage-wrap').html(message);

		$('.beo-alert-wrap #alertCloseBt').click(function(){
			$('.beo-alert-wrap .alertWindow-wrap').hide();
			if( $.type(callback) == 'function') callback();
		});
		
		setTimeout(function(){
			$('.beo-alert-wrap #alertCloseBt').focus();	
		},10);		
	}
	...
	function confirmDialog(message, okCallback, cancelCallback){
		msgInit();
		$('.beo-alert-wrap .confirmWindow-wrap').show();
		
		var newMessage = message.replace('\n','<br>');
		$('.beo-alert-wrap .Massage-wrap').html(message);

		$('.beo-alert-wrap #confirmOkBT').click(function(){
			$('.beo-alert-wrap .confirmWindow-wrap').hide();
			if( $.type(okCallback) == 'function') okCallback();
		})

		$('.beo-alert-wrap #confirmCancelBT').click(function(){
			$('.beo-alert-wrap .confirmWindow-wrap').hide();
	...
	function confirmOpts(opts){
		...
		var buttonHtml = '<button type="reset" class="beo-button"><i>취소</i></button>';		
